...
def _():
    ...
    within_r = {
        name: g["bill_length_mm"].corr(g["bill_depth_mm"])
        for name, g in df.groupby("species", observed=True)
    }
    print(
        f"{len(df)} penguins with both bill measurements "
        f"(dropped {len(penguins) - len(df)} rows)"
    )
    print(f"pooled r = {pooled_r:.2f}")
    print(
        "within species: "
        + ", ".join(f"{name} {r:.2f}" for name, r in within_r.items())
    )
    return (
        aes,
        df,
...
def _(
    ...
    (
        ...
        + geom_smooth(
            method="lm", se=True, alpha=0.12, size=0.9, show_legend=False
        )
