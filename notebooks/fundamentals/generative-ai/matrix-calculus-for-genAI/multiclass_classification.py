...
def _():
    ...
    def compute_gradients(input_tensor, weight_matrix, label, num_classes):
        ...
        jacobian_auto_grad = torch.autograd.functional.jacobian(softmax_fn, logits)  # Manually compute gradient using the analytical formula
        jacobian_analytical = torch.diag(ps.squeeze()) - ps @ ps.T
        return {'grad_loss_wrt_W_correct': weight_matrix.grad, 'grad_loss_wrt_W_analytical': del_L_wrt_W_analytical, 'jacobian_auto_grad': jacobian_auto_grad, 'jacobian_analytical': jacobian_analytical, 'loss': loss.item()}
    ...
    def plot_results(results, label, title_mode='Correct'):
        ...
        sns.heatmap(_results['grad_loss_wrt_W_correct'].detach().numpy(), annot=True, fmt='.2e', cmap='YlGnBu', cbar=False, ax=axes[0, 0], annot_kws={'size': 20})
        axes[0, 0].set_title('Gradient w.r.t Weight Matrix (AutoGrad)', fontsize=22, weight='bold')  # AutoGrad gradient
        axes[0, 0].tick_params(axis='both', which='both', bottom=False, top=False, left=False, right=False, labelbottom=False, labelleft=False)  # Analytical gradient
        sns.heatmap(_results['grad_loss_wrt_W_analytical'].detach().numpy(), annot=True, fmt='.2e', cmap='coolwarm', cbar=False, ax=axes[0, 1], annot_kws={'size': 20})
        ...
        sns.heatmap(_results['jacobian_auto_grad'].detach().numpy().reshape(3, 3), annot=True, fmt='.2e', cmap='YlGnBu', cbar=False, ax=axes[1, 0], annot_kws={'size': 20})
        ...
        sns.heatmap(_results['jacobian_analytical'].detach().numpy().reshape(3, 3), annot=True, fmt='.2e', cmap='coolwarm', cbar=False, ax=axes[1, 1], annot_kws={'size': 20})
    ...
    plot_results(_results, correct_label, title_mode='Correct')  # Heatmap of gradient w.r.t W (Analytical formula) in scientific notation  # Heatmap of Jacobian (AutoGrad) in scientific notation  # Heatmap of Jacobian (Analytical) in scientific notation  # Adjust layout for heatmaps  # Change to "Incorrect" for incorrect label
    return (
        compute_gradients,
        correct_label,
        incorrect_label,
