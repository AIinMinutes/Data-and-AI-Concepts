...
def _(LinearSegmentedColormap, Normalize, nx, plt):
    def plot_search_tree(graph, num_tokens_to_generate, beam_width, title='Search Tree'):
        ...
        fig, ax = plt.subplots(figsize=(3 + 1.15 * beam_width ** num_tokens_to_generate, max(5, 2 + num_tokens_to_generate)), dpi=300)
        ...
        labels = {_node: f"{data['token'].split('_')[0]}\n{data['sequencescore']:.2f}" for _node, data in graph.nodes(data=True)}
...
def _(get_log_probability, graph, model, tokenizer, torch):
    def greedy_search(input_ids, start_node, num_tokens_to_generate=1):
        ...
        for i in range(num_tokens_to_generate):
            ...
            graph.nodes[current_node].update({'sequencescore': cumulative_score / len(input_ids.squeeze()), 'token': next_token + f'_{num_tokens_to_generate - i}'})
        return input_ids
    ...
    def beam_search(input_ids, start_node, num_tokens_to_generate, beam_width):
        ...
        while stack:
            ...
            for i in range(len(top_token_ids) - 1, -1, -1):
                ...
                graph.nodes[current_node].update({'sequencescore': new_score / len(new_input_ids.squeeze()), 'token': token + f'_{tokens_remaining}_{i}'})
