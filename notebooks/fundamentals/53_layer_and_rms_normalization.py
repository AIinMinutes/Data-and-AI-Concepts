...
def _(batch_input_post_layer_norm, batch_input_post_rms_norm, np, plt):
    ...
    plt.axhline(_layer_norm_mean + _layer_norm_std, color='magenta', linestyle=':', linewidth=2, alpha=1.0, label='LayerNorm ± Std')
    ...
    plt.axhline(_rms_norm_mean + _rms_norm_std, color='teal', linestyle=':', linewidth=2, alpha=1.0, label='RMSNorm ± Std')
...
def _(
    ...
    # Loop through each token (i.e., token 0, token 1, token 2, etc.)
        ...
        token_rms_norm = batch_input_post_rms_norm[0, token_index].detach().numpy()  # Extract the token embeddings after LayerNorm and RMSNorm for each token
        ...
        _rms_norm_std = batch_input_post_rms_norm[0, token_index].std().detach().numpy()  # Calculate the mean and standard deviation for each token's embeddings
        ...
        ax.plot(_x, token_rms_norm, 'd', label='RMSNorm', markersize=8, color=rms_norm_color)  # Create the x-axis values (embedding dimension)
        ax.axhline(_layer_norm_mean, color=layer_norm_color, linestyle='--', linewidth=2, alpha=0.7, label='LayerNorm Mean')
        ax.axhline(_layer_norm_mean + _layer_norm_std, color=layer_norm_color, linestyle=':', linewidth=2, alpha=0.7, label='LayerNorm ± Std')
        ax.axhline(_layer_norm_mean - _layer_norm_std, color=layer_norm_color, linestyle=':', linewidth=2, alpha=0.7)  # Select the appropriate subplot (2x2 grid)
        ax.axhline(_rms_norm_mean, color=rms_norm_color, linestyle='--', linewidth=2, alpha=0.7, label='RMSNorm Mean')
        ax.axhline(_rms_norm_mean + _rms_norm_std, color=rms_norm_color, linestyle=':', linewidth=2, alpha=0.7, label='RMSNorm ± Std')
        ax.axhline(_rms_norm_mean - _rms_norm_std, color=rms_norm_color, linestyle=':', linewidth=2, alpha=0.7)  # Define distinct colors for LayerNorm and RMSNorm
