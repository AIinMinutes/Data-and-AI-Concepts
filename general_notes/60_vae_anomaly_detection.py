...
def _():
    ...
    X_train, X_test, y_train, y_test = generate_data(
        n_train=n_train, n_test=n_test, n_features=n_features, contamination=contamination, random_state=1
    )

    # Train the VAE model
    clf_name_vae = "VAE"
    vae_clf = VAE(epoch_num=30, contamination=contamination, beta=1.0)
    vae_clf.fit(X_train)
    return (
        AutoEncoder,
        X_test,
...
def _(
    ...
    def visualize_detailed_results(X, y_true, y_pred, model_name, dataset_name, ax):
        ...
        ax.scatter(
            X[(y_true == 1) & (y_pred == 1), 0],
            X[(y_true == 1) & (y_pred == 1), 1],
            c="red",
            marker="x",
            label="True Positive (Anomaly)",
        )
        ax.scatter(
            X[(y_true == 0) & (y_pred == 0), 0],
            X[(y_true == 0) & (y_pred == 0), 1],
            c="green",
            marker="+",
            label="True Negative (Non-Anomaly)",
        )
        ax.scatter(
            X[(y_true == 0) & (y_pred == 1), 0],
            X[(y_true == 0) & (y_pred == 1), 1],
            c="orange",
            marker="*",
            label="False Positive (Non-Anomaly)",
        )
        ax.scatter(
            X[(y_true == 1) & (y_pred == 0), 0],
            X[(y_true == 1) & (y_pred == 0), 1],
            c="blue",
            marker="^",
            label="False Negative (Anomaly)",
        )
