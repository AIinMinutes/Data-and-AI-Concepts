...
def _(np):
    ...
    anomaly_classes = [(-1, 5), (5, -1), (-2, 2), (6, 6), (-3, 2), (2, -3), (8, 2), (2, 8)]

    anomalies = [(x + np.random.normal(0, 1), y + np.random.normal(0, 1)) for _ in range(3) for x, y in anomaly_classes]
...
def _(anomalies, data, pd):
    ...
    anomalies_df = pd.DataFrame(map(lambda x: list(x) + [1], anomalies), columns=["f1", "f2", "unknown_anomalous"])
    df = pd.concat([df, anomalies_df], axis=0, ignore_index=True)
    df["unknown_anomalous"] = df["unknown_anomalous"].astype("category")
    df.sample(5)
    return (df,)
...
def _(X, mcd, multivariate_normal, np, plt, rv, xx, y, yy, zz, zz2):
    ...
    axs[1, 0].scatter(
        X.iloc[:, 0][y == 0], X.iloc[:, 1][y == 0], c="g", edgecolors="g", marker="*", label="Non-Anomalous", s=150
    )
    axs[1, 0].scatter(
        X.iloc[:, 0][y == 1], X.iloc[:, 1][y == 1], c="orange", marker="*", label="Actual Anomalous", s=150
    )
    axs[1, 0].scatter(
        X.iloc[:, 0][mcd.predict(X) == 1],
        X.iloc[:, 1][mcd.predict(X) == 1],
        c="r",
        marker="x",
