...
def _(
    ...
    axs[0, 0].set_title("Joint Probability Heatmap (Cat-Cat) \n Independent")
    ...
    mi_score_cat_cat = mutual_info_classif(np.array(Y_cat).reshape(-1, 1), X_cat)[0]

    # Plot 2: Contingency Matrix of the Observed Data (Cat-Cat)
    sns.heatmap(contingency_matrix.T, annot=True, cmap="Oranges", fmt="d", cbar_kws={"label": "Count"}, ax=axs[0, 1])
    axs[0, 1].set_title(f"Contingency Matrix (Cat-Cat) \n MI Score: {mi_score_cat_cat:.2f}")
    axs[0, 1].set_xlabel("X (Categories)")
    axs[0, 1].set_ylabel("Y (Categories)")

    # --- Cat-Cont: Non-zero Mutual Information ---
    X = np.random.choice(categories_X, size=100, p=[0.3, 0.4, 0.3])
    means = {1: 0, 2: 5, 3: 10}
    std_dev = 2
    ...
    axs[1, 0].set_title(f"Cat-Cont Scatter Plot\nMI Score: {mi_score_cat_cont:.2f}")
    ...
    mi_score_cont_cont = mutual_info_regression(X_cont.reshape(-1, 1), Y_cont_transformed)[0]
    ...
    scatter_cont_cont = axs[1, 1].scatter(X_cont, Y_cont_transformed, c=Y_cont_transformed, cmap="inferno")
