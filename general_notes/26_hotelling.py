...
def _():
    ...
    data_A = np.random.multivariate_normal(mean_A, cov_A, size=20)
    data_B = np.random.multivariate_normal(mean_B, cov_B, size=25)
    ...
    cov_samp_A = (data_A - mean_samp_A).T @ (data_A - mean_samp_A) / (n_A - 1)
    cov_samp_B = (data_B - mean_samp_B).T @ (data_B - mean_samp_B) / (n_B - 1)

    # Pooled covariance matrix
    cov_pooled = ((n_A - 1) * cov_samp_A + (n_B - 1) * cov_samp_B) / (n_A + n_B - 2)

    # T-squared statistic
    mean_diff = mean_samp_A - mean_samp_B
    T_sq = (n_A * n_B) / (n_A + n_B) * mean_diff.T @ np.linalg.inv(cov_pooled) @ mean_diff
