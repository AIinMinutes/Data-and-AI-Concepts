...
def _(calculate_ewa, np, plt):
    ...
    plt.plot(range(1, n + 1), timeseries, label="Timeseries", color="skyblue", alpha=0.7)
    plt.plot(range(1, n + 1), ewa_series, label=f"EWA (alpha={alpha})", color="orange", linewidth=2)
...
def _(
    ...
    des_fit = ExponentialSmoothing(df["value"], trend="add", seasonal=None).fit(
    ...
    forecast_df = pd.DataFrame(
        {
            ...
            "DES": des_forecast,
            "TES": tes_forecast,
            "Expected value": expected_next_value,
        }
    )
    plt.figure(figsize=(8, 6), dpi=300)
    plt.plot(
        df["date"], df["value"], label="Simulated Time Series", color="white", alpha=0.7, linestyle="--", linewidth=3
    )
    ...
    plt.plot(
        forecast_df["date"], forecast_df["Expected value"], "*", color="white", markersize=12, label="Expected Value"
    )
