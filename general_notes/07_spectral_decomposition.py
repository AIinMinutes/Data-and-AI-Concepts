...
def _():
    ...
    # Given matrix A
    A = np.array([[1, 2, 3], [5, 7, 10], [3, 2, 8], [10, 1, 6]])
...
def _(A, AAt, AtA, S, U, V, np, plt, sigma_2_1, sigma_2_2, sns):
    ...
    sns.heatmap(
        np.diag(sigma_2_1), annot=True, fmt=".2f", cmap="gray_r", ax=axes[1, 2], cbar=False, annot_kws={"size": 20}
    )
    ...
    sns.heatmap(
        np.diag(sigma_2_2), annot=True, fmt=".2f", cmap="gray_r", ax=axes[2, 2], cbar=False, annot_kws={"size": 20}
    )
