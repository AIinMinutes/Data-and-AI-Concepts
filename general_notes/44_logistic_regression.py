...
def _():
    ...
    for feature, coef in zip(features, coefficients):
        ...
        if coef > 0:
            print(
                f"  Interpretation: For each one-unit increase in {feature}, the odds of having diabetes increase by {(odds_ratio - 1) * 100:.2f}%. \n"
            )
        else:
            print(
                f"  Interpretation: For each one-unit increase in {feature}, the odds of having diabetes decrease by {(1 - odds_ratio) * 100:.2f}%. \n"
            )
