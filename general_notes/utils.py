...
def plot_regression_and_heatmap_plots(
    ...
    for i, ((x, y), color) in enumerate(zip(pairs, colors)):
        ...
        sns.regplot(
            x=x, y=y, data=data, ax=ax, scatter_kws={"alpha": alpha, "s": marker_size}, line_kws={"color": color}
        )
    ...
    cbar = sns.heatmap(
        corr_matrix, annot=True, cmap="coolwarm", cbar=True, square=True, ax=ax, annot_kws={"size": annot_size}
    )
    ...
    colorbar.set_label(
        cbar_title, fontsize=cbar_fontsize, labelpad=cbar_title_pad, rotation=270
    )  # Set color bar title and padding
