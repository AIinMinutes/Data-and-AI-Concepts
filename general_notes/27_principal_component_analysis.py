...
def _(np, sklearn_PCA):
    ...
    data_matrix = np.random.multivariate_normal(mean=[3, 2], cov=[[8, 9], [9, 30]], size=100)

    # Standardize data matrix (manual standardization)
    data_matrix_standardized = (data_matrix - data_matrix.mean(axis=0)) / data_matrix.std(axis=0, ddof=1)
...
def _(
    ...
    plt.scatter(
        data_matrix_standardized[:, 0],
        data_matrix_standardized[:, 1],
        alpha=0.7,
        color="white",
        label="Data",
        marker="x",
    ...
    plt.quiver(
        0,
        0,
        sklearn_eigenvecs_sorted[0, :][0],
        sklearn_eigenvecs_sorted[0, :][1],
        color="cyan",
        label="Principal Component-1",
        scale=1 / sklearn_eigenvals_sorted[0],
        scale_units="xy",
    )
    plt.quiver(
        0,
        0,
        sklearn_eigenvecs_sorted[1, :][0],
        sklearn_eigenvecs_sorted[1, :][1],
        color="magenta",
        label="Principal Component-2",
        scale=1 / sklearn_eigenvals_sorted[1],
        scale_units="xy",
    )
    plt.xlim([-3, 3])
    plt.ylim([-3, 3])
