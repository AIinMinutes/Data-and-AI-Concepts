...
def _():
    ...
    D = np.diag(X_std)
    X_reconstructed = Z @ eigenvectors[:, :1].T @ D + X_mean
...
def _(
    ...
    plt.quiver(
        origin[:, 0],
        origin[:, 1],  # Origin points
        eigenvectors[:, 0],
        eigenvectors[:, 1],  # Eigenvector components
        angles="xy",
        scale_units="xy",
        scale=1,
        color=["r", "b"],
    ...
    legend_elements = [Line2D([0], [0], color="r", lw=2, label="PC2"), Line2D([0], [0], color="b", lw=2, label="PC1")]
    ...
    plt.scatter(X[:, 0], X[:, 1], alpha=0.5, label="Original Data", color="magenta")
    plt.scatter(
        X_reconstructed[:, 0], X_reconstructed[:, 1], alpha=0.5, label="Reconstructed Data", marker="x", color="cyan"
    )
    ...
    plt.subplot(2, 2, 4)
    plt.scatter(
        np.arange(len(reconstruction_errors))[:-3],
