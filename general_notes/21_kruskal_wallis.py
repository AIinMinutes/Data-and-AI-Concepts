...
def _():
    ...
    group1, group2, group3 = map(lambda loc: np.random.laplace(loc=loc, scale=5, size=100), [20, 21, 22])
    data = pd.DataFrame(
        {
            "Value": np.concatenate([group1, group2, group3]),
            "Group": np.repeat(["Group 1", "Group 2", "Group 3"], [group1.size, group2.size, group3.size]),
        }
    )
...
def _(data, plt, sns):
    ...
    sns.violinplot(x="Group", y="Value", data=data, hue="Group", palette=["red", "green", "blue"])
