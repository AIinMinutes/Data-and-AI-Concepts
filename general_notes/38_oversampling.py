...
def _():
    ...
    X, y = make_moons(n_samples=1000, noise=0.4, random_state=47)
...
def _(
    ...
    def plot_points_with_probabilities(
        X_train, y_train, X_test, y_test, ax, resampler=None, title=None, elev=90, azim=45
    ):
        if resampler:
        ...
        cmap = ListedColormap(plt.cm.coolwarm(np.linspace(0, 1, 10)))

        scatter = ax.scatter(
            X_train[:, 0], X_train[:, 1], prob_class_1, c=prob_class_1, cmap=cmap, edgecolor="none", s=20
        )
    ...
    fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(12, 12), subplot_kw={"projection": "3d"}, dpi=300)

    ax1 = axes[0, 0]
    scatter_class_0 = ax1.scatter(X_train[y_train == 0, 0], X_train[y_train == 0, 1], c="blue", label="Class-0", s=20)
    scatter_class_1 = ax1.scatter(X_train[y_train == 1, 0], X_train[y_train == 1, 1], c="red", label="Class-1", s=20)
    ...
    cbar = fig.colorbar(
        scatter_adasyn, ax=axes.ravel().tolist(), orientation="vertical", fraction=0.02, pad=0.08, cmap=cmap_original
    )
