...
def _():
    ...
    def plot_transformed_vector(ax, matrix, vector, transform_name):
        ...
        ax.quiver(
            0, 0, vector[0], vector[1], angles="xy", scale_units="xy", scale=1, color="cyan", label="Original Vector"
        )
        ax.quiver(
            0,
            0,
    ...
    reflection_matrix = np.array(
        [
            [np.cos(2 * reflection_theta), np.sin(2 * reflection_theta)],
    ...
    axes[1].set_title(
        f"Reflection (at {reflection_theta_deg}°), det(Reflection Matrix) = {np.round(np.linalg.det(reflection_matrix), 1)}"
    )
