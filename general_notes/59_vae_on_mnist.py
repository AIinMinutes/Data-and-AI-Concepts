    ...
    class VAE(nn.Module):
        def __init__(self):
            ...
            self.encoder = nn.Sequential(
                nn.Flatten(), nn.Linear(28 * 28, 128), nn.ReLU(), nn.Linear(128, 64), nn.ReLU()
            )
            # Latent space
...
def _(nn, torch):
    ...
    def loss_function(reconstructed_x, x, mu, logvar, beta=0.9):
        ...
        MSE = nn.functional.mse_loss(reconstructed_x, x.view(-1, 28 * 28), reduction="sum")
...
def _(device, model, plt, torch):
    ...
    latent_space_coords = [[-2, 2], [0, -2.5], [1, 4], [-5, 3], [0, -1], [2, 0]]
